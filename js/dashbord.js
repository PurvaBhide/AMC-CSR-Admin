...
document.addEventListener("DOMContentLoaded", function () {
  async function updateDashboard() {
    try {
      ...
      document.getElementById("Projects").innerText = projects?.data?.content?.length || 0;
      document.getElementById("totalDonations").innerText = participants?.data?.content?.length || 0;
      document.getElementById("ngoApplications").innerText = ngos?.data?.content?.length || 0;
      document.getElementById("mediaUploads").innerText = companies?.data?.content?.length || 0;
 document.getElementById("totalResources").innerText = resources?.data?.content?.length || 0;
    document.getElementById("totalCategories").innerText = categories?.data.length || 0;
    document.getElementById("fundIdeas").innerText = fundIdeas?.data?.content?.length || 0;
    document.getElementById("successStories").innerText = successStories?.data?.content?.length || 0;
